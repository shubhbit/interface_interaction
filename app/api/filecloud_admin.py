...
class FileCLoudAdmin(FileCloudBase):
    ...
    def login_admin(self, adminuser, adminpassword):
        admin_login_url = get_admin_login_url()
        login_url = self.build_url(
            admin_login_url, {"adminuser": adminuser, "adminpassword": adminpassword})
    ...
    def create_user(self, username, displayname, email, password, authtype=0, status=1):
        ...
        create_user_url_query = self.build_url(create_user_url, {
                                               "username": username,
